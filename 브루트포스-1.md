...
순열을 전부 구해서 푸는 게 아니다. 
주어진 수열을 순회하면서 a[i-1], a[i]의 값을 비교하고, a[i-1]값이 더 클때 i값을 저장. 
i가 0이 될때는 수열이 완벽한 내림차순이기에, False
a[i-1] 뒤에 값 중에서 a[i-1]보다 큰 값중 가장 작은 값을 바꿔준다. 
a[i-1] 이후의 값은 내림차순으로 정렬되어 있기에 뒤집어 준다.
